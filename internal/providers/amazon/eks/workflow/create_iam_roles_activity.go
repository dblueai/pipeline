...
func generateCFStackName(clusterName string) string {
...
func (a *CreateIamRolesActivity) Execute(ctx context.Context, input CreateIamRolesActivityInput) (*CreateIamRolesActivityOutput, error) {
	...
	stackName := generateCFStackName(input.ClusterName)
	createStackInput := &cloudformation.CreateStackInput{
		ClientRequestToken: aws.String(input.AWSClientRequestToken + stackName),
	...
	if err != nil {
		return nil, pkgCloudformation.NewAwsStackFailure(err, stackName, input.AWSClientRequestToken, cloudformationClient)
	}
